...
| D4 | Tamper-evident audit storage | **Open (owner)** | Readable, exportable, append-only by convention; not tamper-evident | — |
...
| D7 | Device time zone from `Intl`, validated as IANA, sent on start and resume, display falls back to the harbour's zone then UTC. It must not affect duration, ETA, arrival, outcome, suspension, appeal, retention or rate limits. | **Partly resolved — conflict reported** | `packages/shared/src/timezone.ts`; `apps/web/src/state/weather.tsx`; `/auth/time-zone` validates before charging its rate limit. **Conflict:** the approved storm-risk policy v3 counts storm nights (and so risk decisions) in the sender's account zone. That behaviour was not changed: a zone change can move *future* storm nights of new bottles, never past ones, and is limited to four changes a day. Duration, ETA and arrival never read the zone, and changing the clock never reruns an outcome. Accounts whose device never reports a zone still carry no storm risk. Removing the zone from risk needs a risk-policy change (v4) that the owner must approve | `http/device-zone.test.ts`, `domain/journey-pace.test.ts`, `packages/shared/src/timezone.test.ts`, `apps/web/src/state/weather.test.tsx` |
