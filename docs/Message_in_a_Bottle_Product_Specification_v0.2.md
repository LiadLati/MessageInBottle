...
Implemented 2026-09-19 (terms, guidelines, privacy — mechanism only; texts remain a working draft): the three documents are viewable before registration and from account settings; registration needs two separate, initially unchecked decisions (accept the Terms of Use and Community Guidelines; acknowledge the Privacy Policy), enforced by the server, which records each document's version and the acceptance time per account; existing accounts are never treated as having accepted and are asked on sign-in once a released version exists; no marketing consent exists because there is no marketing. The draft texts were corrected against the product; the operator identity, minimum age, legal basis, deployment providers, retention periods, appeal deadline, deletion process and legal review remain open and are marked in the text. A production build refuses registration until the documents are released in code. See docs/LEGAL_DOCUMENTS.md.
