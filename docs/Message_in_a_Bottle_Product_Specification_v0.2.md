...
Implemented 2026-09-21 (terms, community rules, privacy, child safety, account deletion): the documents are published at version 1.0 in English, readable before registration and from account settings in the App, and served as public unauthenticated HTML at /legal/* for store listing use. Registration requires two separate, initially unchecked decisions — agreeing to the Terms of Use and Community Rules, and confirming the Privacy Policy has been read — enforced by the server, which records each document's version and the acceptance time per account; a new version asks every account again before ordinary use while leaving authentication, the documents, standing, appeals, sign-out and deletion reachable. The App has no age gate of any kind and makes no claim that users are adults or age-verified; safety rules concerning minors bind every user. Account deletion is implemented end to end, in the App and on the public web, as one transactional idempotent operation that re-authenticates, revokes sessions immediately, anonymises the account, removes it from discovery and friends, cancels letters still at sea, leaves received letters with their recipients, and retains moderation evidence only where an open report, a pending appeal or an active restriction requires it. See docs/LEGAL_DOCUMENTS.md.
