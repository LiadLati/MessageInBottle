...
import { clearDraft, emptyDraft, loadDraft, saveDraft, type Draft } from '../lib/draft.js';
  ...
  if (step === 'compose') {
    return (
      ...
      >
        ...
        {/* A live region, and the reason the button is disabled (audit A11Y-010). */}
        <p id="seal-reason" className="note" role="status" hidden={validation.ok}>
          {validation.ok
            ? null
        ...
        <button
          ...
          aria-describedby={validation.ok ? undefined : 'seal-reason'}
          onClick={goPreview}
        >
          Seal the letter
        </button>
      ...
      <section className="sheet preview" aria-label="Preview and release">
        ...
        <div className="sheet-footer">
          ...
          <button
            ...
            style={{ marginTop: 12 }}
            disabled={!canRelease}
            aria-describedby={canRelease ? undefined : 'throw-reason'}
            onClick={release}
          >
          ...
          {canRelease ? null : (
            <p id="throw-reason" className="t-meta" style={{ textAlign: 'center', marginTop: 8 }}>
              {!p
                ? 'Waiting for the route.'
                : !p.eligible
                  ? 'This bottle cannot be released right now.'
                  : 'Tick the box above to release.'}
            </p>
          )}
          ...
          {releaseError ? (
            <p className="note error" role="alert" style={{ marginTop: 12 }}>
              {releaseError instanceof ApiError && releaseError.code === 'release_rejected'
                ...
                : `The sea did not take it: ${releaseError.message.replace(/[.!?]+$/, '')}.`}{' '}
